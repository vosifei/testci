...
class MailerPatchIssuesTest < Redmine::IntegrationTest
  ...
  def test_mailer_patch_issue_attachments
    ...
    cf = IssueCustomField.generate!(:name => 'a Test Field', :field_format => 'bool')
    cf.save
    ...
    files << uploaded_test_file('testfile.txt', 'text/plain')
        ...
        post '/projects/ecookbook/issues', :params => {
            :issue => {
              ...
              :attachments => {
                ...
                '2' => {'file' => files[1], 'description' => 'This is an attachment'}
              }
          }
      end
    end
